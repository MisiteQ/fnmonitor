...
| 版本 | 内容 |
|---|---|
| v2.9.4 | 设置页分组重排（界面外观 / 数据 / 服务 / 更新与关于四组）；数据导出移入设置页「数据」组；新增云纹背景装饰（半透明祥云纹理，随主题自适应，可随时开关并跨设备同步） |
| v2.9.3 | 软件更新模块合并重构；修复「下载到本电脑」（iframe 弹窗拦截）与「下载到 NAS」（支持自定义保存文件夹）；新增「立即更新」一键升级与「自动更新」开关（自动下载+安装+重启，带备份回滚）；字体默认=2.8.4 原始大小 |
| v2.9.2 | 内存明细堆叠横条加粗（9→14px，图例色块同步加大）；新增「字体大小」设置：90%/100%/110%/125% 四档整体缩放，即时生效并自动同步 NAS 端界面配置 |
| v2.9.1 | 界面设置同步与数据保存目录合并（移除重复的「修改保存目录」按钮）；数据目录输入框加宽并新增「取消」还原按钮 |
| v2.9.0 | 新增「关于」（软件简介 / 作者 GitHub / 项目地址）与「软件更新」：GitHub Releases 在线检查更新、新版红点提示、安装包下载到 NAS 或经 NAS 代理下载到电脑、可选每 6 小时自动检查并自动下载；明确所有数据保存在 NAS 服务器端，移除浏览器端天气位置存储 |
| v2.8.5 | 界面设置同步区块显示同步数据（ui.json）当前实际保存路径，新增「修改保存目录」按钮一键跳转到数据目录设置 |
| v2.8.4 | 大幅降低 CPU / 内存占用：历史库清理降为每小时、重采集降频、接口结果缓存复用（多设备不再成倍消耗）；HTTP 长连接 + 限制 glibc arena 解决内存虚高；后台标签页自动暂停轮询 |
